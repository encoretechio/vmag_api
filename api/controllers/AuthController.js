...
module.exports = {
  loginx: function (req, res) {
    ...
    User.findOneByEmail(req.body.email).populate("company").exec(function (err, user) {
      if (err) {
      ...
      user.isPasswordValid(req.body.password, function (err, bool) {
        ...
        if (bool) {
          //When user is valid populate company and it's attributes.
          return Company.findOne(user.company)
            .populate('video')
            .exec((err, company) => {
